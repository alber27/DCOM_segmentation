# -*- coding: utf-8 -*-
"""
Created on Wed May  8 15:03:00 2019
...
# Load all scan with the given path
def load_scans(path):
    slices = [dicom.read_file(path + '/' + s) for s in os.listdir(path)]
    slices.sort(key=lambda x: int(x.InstanceNumber))
    return np.stack([s.pixel_array for s in slices])

# Return a list of matrix[n, channel(1), height, width] already processed and normalized
# Implements: load_scans, process_scans
...
# Normalize the image matrix(0-255)
...
# Visualize the images previously normalized 
...
def get_masks(scans,masks_list):
    ...
    for i_m in range(len(masks_list)): # 0 to 6 (len masks_list)
        for i in range(-masks_list[i_m][3],masks_list[i_m][3]+1): # -7 to 8 
            for j in range(-masks_list[i_m][3],masks_list[i_m][3]+1): # -7 to 8
...
img_rows = 256
img_cols = 256
scan_folder = './lung_cancer/stage1_full/'
...
a = load_scans(scan_folder+df.id[125])
a[100]
